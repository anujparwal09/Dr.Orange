  ...
  useEffect(() => {
    const initGoogleSignIn = () => {
      if (window.google && GOOGLE_CLIENT_ID) {
        ...
        const googleButtonDiv = document.getElementById('google-login-btn');
        if (googleButtonDiv) {
      ...
      >
        ...
        <div className="text-center mb-8">
          <svg width={52} height={52} viewBox="0 0 38 38" className="mx-auto">
            <defs>
              <radialGradient id="login-og" cx="38%" cy="32%">
                <stop offset="0%" stopColor="#FFD580" />
                <stop offset="45%" stopColor="#FF8C00" />
                <stop offset="100%" stopColor="#CC4400" />
              </radialGradient>
            </defs>
            <circle cx="19" cy="23" r="13" fill="url(#login-og)" />
        ...
        <div className="font-playfair text-[26px] font-bold text-center mb-2">Welcome back</div>
        <div className="text-sm text-center mb-8" style={{ color: 'var(--muted)' }}>
          Sign in to Dr. Orange to access your scan history and reports
        </div>
        ...
        <div className="text-center mt-6 text-[13px]" style={{ color: 'var(--muted)' }}>
          ...
          <Link href="/signup" className="no-underline" style={{ color: 'var(--orange)', cursor: 'pointer' }}>
            Sign up →
          </Link>
        </div>
      </motion.div>
